...
// Termo de Entrega e Aceite — versão do romaneio para o CLIENTE assinar,
// confirmando os equipamentos que está recebendo e que está de acordo.
...
export default function ReciboEntregaPage() {
  ...
  // Só equipamentos físicos entram no termo (serviços não são entregues)
    ...
    <div className="min-h-screen bg-slate-100 print:bg-white">
      <div className="print:hidden sticky top-0 z-10 bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between">
        <p className="text-sm font-medium text-slate-700">
          Termo de Entrega e Aceite — OS #{orc?.numero}
        </p>
        <button
      ...
      <div className="mx-auto my-6 print:my-0 bg-white shadow print:shadow-none w-[210mm] min-h-[297mm] px-[12mm] py-[10mm] text-[10.5px] leading-snug text-slate-800">
        ...
        <div className="flex items-center justify-between border-b-2 border-slate-800 pb-3">
          ...
          <div className="text-right">
            <p className="text-[15px] font-bold">TERMO DE ENTREGA E ACEITE</p>
        ...
        <div className="grid grid-cols-2 gap-3 mt-3 border border-slate-300 rounded p-3">
          <div>
            ...
            {orc?.cliente?.razaoSocial && (
              <p><span className="font-bold">Razão social:</span> {orc.cliente.razaoSocial}</p>
            )}
          ...
          <div>
            <p>
              <span className="font-bold">Período do evento:</span> {fmtData(orc?.dataInicio)}
              {orc?.dataFim ? ` a ${fmtData(orc.dataFim)}` : ""}
            </p>
            <p><span className="font-bold">Data da entrega:</span> ____/____/____ às ____:____</p>
          </div>
        </div>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-slate-100 text-[9px] uppercase text-slate-600">
                  ...
                  <th className="border border-slate-300 px-1.5 py-1 w-16 text-center">Recebido ✓</th>
                  <th className="border border-slate-300 px-1.5 py-1 w-32 text-left">Estado / Obs.</th>
                </tr>
              </thead>
              <tbody>
              ...
              <tbody>
                {extras.map((e: any) => (
                  <tr key={e.id} className="text-red-700">
                    ...
                    <td className="border border-slate-300 px-1.5 py-1.5 w-16 text-center text-slate-300">☐</td>
                    <td className="border border-slate-300 px-1.5 py-1.5 w-32"></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div
          className="mt-5 border border-slate-300 rounded p-3 text-[9.5px] text-slate-600"
          style={{ breakInside: "avoid" }}
        >
          <p className="font-bold text-[10px] uppercase text-slate-700 mb-1">
            Declaração de Recebimento e Aceite
          </p>
          <p>
            Declaro que <span className="font-bold">RECEBI</span> os equipamentos acima
            relacionados, nas quantidades indicadas, em perfeito estado de funcionamento e
            conservação, e que <span className="font-bold">ESTOU DE ACORDO</span> com a
            relação apresentada. Comprometo-me a zelar pela guarda dos equipamentos durante o
            período do evento e a devolvê-los nas mesmas condições. Em caso de perda,
            extravio ou dano, reconheço o valor de reposição indicado por unidade
            {totalReposicao > 0
              ? ` (valor total de reposição desta entrega: ${fmtValor(totalReposicao)})`
              : ""}
            . Eventuais ressalvas devem ser anotadas na coluna &quot;Estado / Obs.&quot; no ato
            do recebimento.
          </p>
          <div className="grid grid-cols-2 gap-8 mt-10">
            <div className="text-center">
              <div className="border-t border-slate-400 pt-1">
                <span className="font-bold">CLIENTE (recebedor)</span>
                <br />
                Nome: _______________________________
                <br />
                Doc.: ________________ Data: ____/____/____
              </div>
            </div>
            <div className="text-center">
              <div className="border-t border-slate-400 pt-1">
                <span className="font-bold">{empresa?.name || "Empresa"} (entregador)</span>
                <br />
                Nome: _______________________________
                <br />
                Doc.: ________________ Data: ____/____/____
